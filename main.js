...
global.authFile = `morocco-ai-session`
...
function purgeSession() {
    let prekey = []
    let directorio = readdirSync("./morocco-ai-session")
    let filesFolderPreKeys = directorio.filter(file => {
        return file.startsWith('pre-key-')
    })
    prekey = [...prekey, ...filesFolderPreKeys]
    filesFolderPreKeys.forEach(files => {
    unlinkSync(`./morocco-ai-session/${files}`)
})
...
const directories = ['./morocco-ai-session/', './jadibts/']
...
let bio = `Morocco OpenAI BOT Made by Morocco Openai teams ಠ⁠‿⁠ಠ `
